...
function monkeyPatchMediaDevices() {
    document.onreadystatechange = (event) => {
      ...
      setWebContainer();
      createAudioElement();
      setModeNone(window.classActivated);
    }
  };
          ...
          try {
            ...
            if (response && response.farewell && window.peerConection) {
              ...
              if (currentMediaStreamTracks) {
                const currentTrackLabel =
                  currentMediaStreamTracks[0].track.label;
                let run = false;
        ...
        async function (response) {
          if (response && response.farewell) {
            if (response.farewell != defaultMode) {
              ...
              if (window.assignModes) window.assignModes();
            }
          }
        }
      );
  ...
  const setMediaStreamTracks = async () => {
    try {
      ...
      const defaultDevice = videoDevices.filter(
        (d) =>
          d.deviceId == defaultVideoId || d.deviceId.exact == defaultVideoId
      );
